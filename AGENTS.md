...
Before comparing specialization matrices, run `npm run query:matrix-coverage`. It explicitly distinguishes full seasonal matrices from capability-only records and lists missing or extra dungeon IDs; do not infer matrix completeness from the presence of a capability record.
