...
Patch 12 collector observations cannot safely expose enemy GUIDs inside a key. Their pulls therefore carry banked scenario-progress deltas with `enemyIdentityStatus: unavailable-secret-values`. Comparisons omit enemy-count drift instead of interpreting an empty identity list as zero enemies. Pull-to-route matching remains order-inferred unless an out-of-game importer assigns a reviewed stable pull ID.
