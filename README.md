...
Canonical utility records and complete eight-dungeon matrices cover 27 specializations: Restoration and Enhancement Shaman plus every Hunter, Paladin, Druid, Priest, Mage, Death Knight, Warrior, and Rogue specialization. Composition queries preserve scope, action-level talent availability, configuration requirements, and distinctions between interrupts, stops, dispels, self-cleanses, group buffs, temporary health, damage reduction, and positional utility. The season affix set is machine-readable across keystone thresholds. Canonical IDs are versioned and sourced; uncorroborated discoveries remain drafts. The examples under `examples/` are synthetic and must never be treated as game facts.
