...
Canonical utility records and complete eight-dungeon matrices cover 37 specializations: every Shaman, Hunter, Paladin, Druid, Priest, Mage, Death Knight, Warrior, Rogue, Warlock, Monk, and Demon Hunter specialization. Composition queries preserve scope, action-level talent availability, configuration requirements, and distinctions between interrupts, stops, dispels, self-cleanses, group buffs, enemy damage-taken increases, temporary health, damage reduction, movement tools, and consumable supply. The season affix set is machine-readable across keystone thresholds. Canonical IDs are versioned and sourced; uncorroborated discoveries remain drafts. The examples under `examples/` are synthetic and must never be treated as game facts.
