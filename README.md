...
Version 1.14 is a validated, queryable foundation. It includes:
...
Canonical utility records and complete eight-dungeon matrices cover all 40 current Retail specializations across every class. Composition queries preserve scope, action-level talent availability, configuration requirements, and distinctions between interrupts, stops, Magic dispels, Bleed cleanses, other dispel domains, group buffs, enemy damage-taken increases, temporary health, damage reduction, movement tools, and consumable supply. The season affix set is machine-readable across keystone thresholds. Canonical IDs are versioned and sourced; uncorroborated discoveries remain drafts. The examples under `examples/` are synthetic and must never be treated as game facts.
...
Elemental Shaman completes the Shaman class and extends the catalog to 34 full matrices. Its 12.1 record separates Spirit Walk's self-only root removal, Jet Stream's talent-dependent group snare removal, Tremor Totem's Fear/Charm/Sleep cleanse, and Thunderstorm's displacement stop. Wind Rush Totem is consistently typed as group movement rather than a generic buff.

Demon Hunter coverage reflects Midnight's current three-spec roster, including Devourer (`specId` 1480). The shared 12.1 model keeps Burn It Out and Soul Cleanse self-only, treats Chaos Brand as an enemy Magic-damage-taken increase instead of a friendly buff, and preserves Void Nova, Chaos Nova, and Imprison as control rather than school-lockout interrupts. Vengeance separately models Sigil of Silence, Sigil of Chains, and Fiery Brand's current self-defensive behavior.
...
An independently generated audit-coverage catalog reports how deeply each specialization's guide sources have been checked. Across all 40 current specs, six are partially audited and 34 are provenance-only; none are yet labeled fully audited, and every matrix has at least one source URL. Devastation and Augmentation Evoker now each preserve 32 current Wowhead utility-panel mentions across all eight dungeons without inventing numerical ratings. Their Blinding Vale matrices also restore Poison cleansing against canonical Toxic Spew while retaining the rejection of unrelated Maisara Caverns claims. Provenance-only means WHELP knows where guidance came from, not that every source claim has been independently corroborated. Run `npm run query:source-audit-coverage`, optionally filtering by `--level` or `--spec`, and see `docs/SOURCE_AUDITS.md` for promotion rules.
...
The capability catalog is marked complete for the current 40-spec Retail roster in `data/specs/coverage.json`. Query output still includes completeness state and modeled-spec count so future roster changes cannot silently turn missing data into “has no utility.”
